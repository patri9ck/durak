  ...
  "DefendCommand" should {
    "execute()" should {
      "remove the used card from the defending player and add it to the used cards and take the undefended to the defended" in {
        ...
        controller.status = Status(List(Player("Player1", List(Card(Rank.Ace, Suit.Spades), Card(Rank.Seven, Suit.Spades)), Turn.Defending),
      ...
      "set the defending player to finished if he finished" in {
        ...
        controller.status = Status(List(Player("Player1", List(Card(Rank.Ace, Suit.Spades)), Turn.Defending),
